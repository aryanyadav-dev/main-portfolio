...
export function ExperienceItem({ title, role, children, collapsible = false, link, collapsedHeight = "max-h-20", titleClassName = "" }: ExperienceItemProps) {
    const [isExpanded, setIsExpanded] = useState(false);

    return (
        <div className="group" data-collapsed={collapsible && !isExpanded ? "true" : "false"}>
            <div className="mb-2 flex flex-col justify-between sm:flex-row sm:items-baseline">
                <div className="flex items-center gap-2">
                    <span className={`font-medium text-black dark:text-white ${titleClassName}`}>{title}</span>
                    {link && (
                        <a
                            href={link}
                            target="_blank"
